...
format:
  clean-revealjs:
    ...
    # Fond de la diapo de titre
    # title-slide-attributes:
      # data-background-image: img/logo_hesav_black.png # (ou supprime cette ligne si tu veux fond uni)
      # data-background-position: center
      # data-background-size: cover

    # Navigation
    menu:
      side: left # menu latéral à gauche (facultatif)
    ...
    # Liens cliquables dans la prévisualisation
    preview-links: auto

    # Tableau blanc interactif (chalkboard)
    chalkboard:
      buttons: true # cacher les boutons, activer via clavier
      theme: whiteboard # fond clair, compatible avec thème HESAV
...
-   **Rseek** — moteur de recherche spécialisé, ciblant la documentation, les blogs et les forums R : [rseek.org](https://rseek.org/)\
...
-   Cocher : *Use native pipe operator, \|\>*\
    ...
    Permet d’utiliser [`|>`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/pipeOp.html) au lieu de [`%>%`](https://magrittr.tidyverse.org/reference/pipe.html) du package [magrittr](https://magrittr.tidyverse.org/index.html).
...
-   Cocher : *Show margin*\
    → affiche une ligne verticale (souvent à 80 caractères) pour favoriser un code lisible.
...
C’est cette association à un nom qui permet ensuite de le retrouver et réutiliser.
:::

## Premiers pas dans R — Affectation et opérateur `<-`

Saisir le texte suivant dans la console, puis appuyer sur <kbd>↵ Enter</kbd> :
...
Avec la fonction [`c()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/c.html) , on peut **c**ombiner plusieurs éléments séparés par une virgule :
...
"Le nom de l’objet reçoit la valeur" ou "Stocker ce qu'il y a à droite de `<-` dans un objet nommé à gauche"
...
::: callout-warning
Les noms d’objets peuvent contenir des lettres, des chiffres, ainsi que les symboles `.` et `_`, mais ne doivent pas commencer par un chiffre.
...
L’effacer de l’environnement R avec la fonction [`rm()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/rm.html) ([`remove()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/rm.html)) :
...
-   [`data.frame()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/data.frame.html) : crée un tableau de données (*data frame*).\
-   [`mean()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/mean.html) : calcule la moyenne arithmétique.\
-   [`sd()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/sd.html) : calcule l’écart-type (la dispersion autour de la moyenne).

## Premiers pas dans R — Utilisation de fonctions II

On va construire une table de données à l’aide de la fonction [`data.frame()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/data.frame.html), qui regroupe plusieurs variables dans un même objet.
...
L’opérateur [`$`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/Syntax.html) permet d’accéder à une colonne (une variable) d’un tableau de données (*data frame*) ou à un élément nommé d’une liste.

Appliquons maintenant les fonctions [`mean()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/mean.html) et [`sd()`](https://stat.ethz.ch/R-manual/R-devel/library/stats/html/sd.html) à la variable `taille` du *data frame* `d` :
...
-   Pour certaines fonctions comme `mean()`, `median()`, `sd()` ou `min()`, ajoutez l’argument `na.rm = TRUE` (*NA remove*) pour ignorer les valeurs manquantes.

-   Sinon, si la variable contient des `NA`, le résultat renvoyé par R sera `NA` (`na.rm = FALSE` par défaut).
...
Ou en ligne de commande, avec la fonction [`install.packages()`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/install.packages.html) :
...
Une fois installés, les packages doivent être chargés dans l’environnement R pour être utilisés avec la fonction [`library()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/library.html) :
...
-   Utilisez [`?fonction`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/help.html) ou [`help("fonction")`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/help.html) pour accéder à la documentation d’une fonction.\
-   Pour explorer un package complet : `help(package="nom_du_package")`.\
-   Recherchez des vignettes via [`browseVignettes()`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/browseVignettes.html) ou [`vignette("nom")`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/vignette.html) pour des guides illustrés.\
-   Utilisez des fonctions de recherche interne : [`apropos()`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/apropos.html), [`help.search()`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/help.search.html) (alias [`??`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/help.search.html)) et [`RSiteSearch()`](https://stat.ethz.ch/R-manual/R-devel/library/utils/html/RSiteSearch.html) pour explorer plus largement.\
...
-   <kbd>⌘</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd> (macOS)
...
Voici un exemple de structure de script : [⬇️ Télécharger le modèle de script](script_template.R)
...
Ou exécuter la fonction [`getwd()`](https://stat.ethz.ch/R-manual/R-devel/library/base/html/getwd.html) :
...
-   Conseillé de rassembler tous vos fichiers au même endroit lorque vous ne travaillez pas dans un projet R

-   Définir un répertoire de travail par défaut (*working directory*).

Exemple personnel :

-   J'ai crée un dossier structuré comme suit : `Code → R → sandbox` dans mon dossier *Documents* (`~/Documents/Code/R/sandbox`).
...
::: {.callout-tip title="Astuce : un projet = un dossier = une analyse"}
Adoptez dès le départ la bonne habitude : un dossier par projet — clair, isolé, reproductible.
:::
...
[⬇️  init_project.R](init_project.R)

**Important :** exécutez ce script <span style="color:red">**depuis le répertoire du projet**</span> (ou après avoir ouvert le projet `.Rproj` dans RStudio), afin que les dossiers soient créés au bon endroit.
:::
...
Le package [`rio`](https://gesistsa.github.io/rio/) simplifie grandement l’importation et l’exportation de fichiers de données dans R.
...
Vous pouvez aussi redémarrer la session R (**Session → Restart R**) : cela vide l’environnement et décharge tous les packages chargés.  

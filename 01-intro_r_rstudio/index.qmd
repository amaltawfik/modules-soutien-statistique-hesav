...
-   Les commandes R sont généralement simples à comprendre et ne nécessitent aucune expérience préalable en programmation pour débuter
...
## Ressources clés pour l’écosystème R {.smaller}

-   **Site officiel du projet R** : [r-project.org](https://www.r-project.org/)\
...
-   **METACRAN** — moteur de recherche et visualisation des packages CRAN : [r-pkg.org](https://www.r-pkg.org/)\
...
-   **Site du tidyverse** — un ensemble cohérent de packages conçus pour la science des données avec R : [tidyverse.org](https://www.tidyverse.org/)
...
## Configurations de RStudio — Général {.smaller}
...
## Configurations de RStudio — Code Editing {.smaller}
...
-   Cocher : *Use native pipe operator, \|\>*\
    ...
    Permet d’utiliser [`|>`](https://rdrr.io/r/base/pipeOp.html) au lieu de [`%>%`](https://magrittr.tidyverse.org/reference/pipe.html) du package [magrittr](https://magrittr.tidyverse.org/index.html).
...
::: {.callout-tip title="Objectif" icon="false"}
Faciliter l’édition du code, réduire les erreurs de syntaxe et adopter la syntaxe R moderne avec le pipe natif [`|>`](https://rdrr.io/r/base/pipeOp.html).
:::
...
-   Choisir : *R code formatting* → **styler**\
    → sélectionne le package [styler](https://styler.r-lib.org/) comme outil de formatage du code en appliquant automatiquement les conventions de [style du tidyverse](https://style.tidyverse.org/) (indentation cohérente, espaces normalisés, code plus lisible).
...
::: {.callout-tip title="Objectif" icon="false"}
Assurer un code clair, cohérent et bien indenté grâce au formatage automatique géré par [styler](https://styler.r-lib.org/) à chaque sauvegarde.
:::
...
-   Le thème [Dracula](https://draculatheme.com/rstudio)\

-   D'autres [ici](https://github.com/topics/rstheme) et [là](https://github.com/mkearney/rstudiothemes)
...
::: {.callout-tip title="À retenir" icon="false"}
Dans R, tout ce que vous créez, importez ou calculez devient un objet stocké en mémoire, visible dans l’onglet **Environment** de RStudio — à condition de lui avoir donné un nom.

C’est cette association à un nom qui permet ensuite de le retrouver et de le réutiliser.
:::

## Premiers pas dans R — Affectation et opérateur `<-`

Saisir la commande suivante dans la console, puis appuyer sur <kbd>↵ Enter</kbd> :
...
Avec la fonction [`c()`](https://rdrr.io/r/base/c.html) , on peut **c**ombiner plusieurs éléments séparés par une virgule :
...
-   Tous les objets créés apparaissent automatiquement dans le volet **Environment** de RStudio, d’où ils peuvent être visualisés ou supprimés.
...
L’effacer de l’environnement R avec la fonction [`rm()`](https://rdrr.io/r/base/rm.html) ([`remove()`](https://rdrr.io/r/base/rm.html)) :
...
Voici trois fonctions très utiles pour débuter :

-   [`data.frame()`](https://rdrr.io/r/base/data.frame.html) : crée un tableau de données (*data frame*).\
-   [`mean()`](https://rdrr.io/r/base/mean.html) : calcule la moyenne arithmétique.\
-   [`sd()`](https://rdrr.io/r/stats/sd.html) : calcule l’écart-type (la dispersion autour de la moyenne).

## Premiers pas dans R — Utilisation de fonctions II

On va construire une table de données à l’aide de la fonction [`data.frame()`](https://rdrr.io/r/base/data.frame.html), qui regroupe plusieurs variables dans un même objet.
...
[`NA`](https://rdrr.io/r/base/NA.html) (*Not Available*) indique une valeur manquante, qu’il s’agisse d’un nombre, d’une chaîne de caractère, etc.

```{r, echo = F}
d
```

Il existe aussi [`NaN`](https://rdrr.io/r/base/is.finite.html) (*Not a Number*), qui signale une valeur numérique indéfinie (ex. 0/0) :
...
L’opérateur [`$`](https://rdrr.io/r/base/Syntax.html) permet d’accéder à une colonne (une variable) d’un tableau de données (*data frame*) ou à un élément nommé d’une liste.

Appliquons maintenant les fonctions [`mean()`](https://rdrr.io/r/base/mean.html) et [`sd()`](https://rdrr.io/r/stats/sd.html) à la variable `taille` du *data frame* `d` :
...
-   Pour certaines fonctions comme [`mean()`](https://rdrr.io/r/base/mean.html), [`median()`](https://rdrr.io/r/stats/median.html), [`sd()`](https://rdrr.io/r/stats/sd.html), [`min()`](https://rdrr.io/r/base/Extremes.html) ou [`max()`](https://rdrr.io/r/base/Extremes.html), ajoutez l’argument `na.rm = TRUE` (*NA remove*) pour ignorer les valeurs manquantes (`na.rm = FALSE` par défaut).
...
Ou en ligne de commande, avec la fonction [`install.packages()`](https://rdrr.io/r/utils/install.packages.html) :
...
Une fois installés, les packages doivent être chargés dans l’environnement R pour être utilisés avec la fonction [`library()`](https://rdrr.io/r/base/library.html) :
...
L’environnement RStudio permet de compléter automatiquement le code avec la touche <kbd>Tab ↹</kbd>

-   Saisissez les premières lettres d’une fonction, d’un objet ou d’un argument\
-   Appuyez sur <kbd>Tab ↹</kbd> pour afficher les suggestions de complétion\
-   Utilisez ensuite les flèches <kbd>↑</kbd> et <kbd>↓</kbd> pour naviguer dans la liste\
-   Validez votre choix avec <kbd>↵ Enter</kbd> ou continuez à taper

*Exemple :*\
Tapez `mea` puis appuyez sur <kbd>Tab ↹</kbd> → RStudio suggère `mean()`.
...
Pour insérer un chemin d’accès à un fichier, placez le curseur entre les guillemets doubles (`""`) puis appuyez sur <kbd>Tab ↹</kbd>.

-   RStudio affichera alors tous les dossiers et fichiers disponibles dans le répertoire de travail.

-   Vous pouvez naviguer avec les flèches <kbd>↑</kbd> <kbd>↓</kbd> ou la souris, ouvrir un dossier avec <kbd>Tab ↹</kbd>, et valider la sélection avec <kbd>↵ Enter</kbd>.

Vous pouvez également accéder à des fichiers situés en dehors du répertoire actuel en commençant à taper le début de leur chemin d’accès (par exemple `"/Users/"` ou `"C:/"`) et en plaçant le curseur entre `/` et `"`puis en appuyant sur <kbd>Tab ↹</kbd> .

## Obtenir de l’aide dans R

-   Utilisez [`?fonction`](https://rdrr.io/r/utils/help.html) ou [`help("fonction")`](https://rdrr.io/r/utils/help.html) pour accéder à la documentation d’une fonction.\
-   Pour explorer un package complet : `help(package="nom_du_package")`.\
-   Recherchez des vignettes via [`browseVignettes()`](https://rdrr.io/r/utils/browseVignettes.html) ou [`vignette("nom")`](https://rdrr.io/r/utils/vignette.html) pour des guides illustrés.\
-   Utilisez des fonctions de recherche interne : [`apropos()`](https://rdrr.io/r/utils/apropos.html), [`help.search()`](https://rdrr.io/r/utils/help.search.html) (alias [`??`](https://rdrr.io/r/utils/help.search.html)) et [`RSiteSearch()`](https://rdrr.io/r/utils/RSiteSearch.html) pour explorer plus largement.\
-   Consultez le portail officiel : [Getting Help with R](https://www.r-project.org/help.html) — documentation, FAQ, mailing-lists, forums.\
...
Ajouter le caractère oublié (puis <kbd>↵ Enter</kbd>) ou appuyer sur <kbd>Esc</kbd> (<kbd>Échap</kbd>) pour annuler et recommencer.
...
-   <kbd>↑</kbd> rappelle la ou les commandes précédentes\
-   <kbd>↓</kbd> avance dans l’historique\
-   <kbd>Ctrl</kbd> + <kbd>↑</kbd> (Windows / Linux) ou <kbd>⌘</kbd> + <kbd>↑</kbd> (macOS) ouvre tout l’historique pour parcourir la liste complète de vos commandes récentes
...
options(width = 75)
set.seed(123)  # pour reproduire le même tirage
...
-   Sur la deuxième ligne, \[18\] signifie que le premier nombre de cette ligne est le 18ᵉ élément du vecteur

-   Sur la dernière ligne, \[35\] signifie que le premier nombre affiché est le 35ᵉ
...
3.  Enregistrer le script via l’icône 💾 ou **File → Save/Save as**\
    (les fichiers portent l’extension `.R`).

Dans un script, la touche <kbd>↵ Enter</kbd> insère une nouvelle ligne, elle n’exécute pas le code (contrairement à la console).
...
-   <kbd>Ctrl</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd> (Windows / Linux) ou <kbd>⌘</kbd> + <kbd>Shift</kbd> + <kbd>C</kbd> (macOS)
...
Voici un exemple de structure de script : <a href="script_template.R" download>⬇️ Télécharger le modèle de script</a>
...
-   Cliquer sur l’icône **Outline** (tout à droite de la barre d’outils du script), ou utiliser le raccourci :
    ...
    -   <kbd>Ctrl</kbd> + <kbd>Maj</kbd> + <kbd>O</kbd> (Windows / Linux) 
...
-   Eviter le code spaghetti : un code désordonné, illisible et non structuré, difficile à réutilisers
...
Ou exécuter la fonction [`getwd()`](https://rdrr.io/r/base/getwd.html) :
...
En pratique, un projet RStudio correspond simplement à un dossier\
créé sur votre disque dur, dans lequel vous placerez :
...
::: {.callout-tip title="Astuce : un projet RStudio = une analyse"}
...
::: {.callout-tip title="Script à télécharger pour créer l'arborescence d'un projet" icon="false"}
Vous pouvez télécharger et exécuter le script suivant pour créer automatiquement la structure de base d’un projet R :

<a href="init_project.R" download>⬇️ init_project.R</a>

**Important :** exécutez ce script [**depuis le répertoire du projet**]{style="color:red"} (ou après avoir ouvert le projet `.Rproj` dans RStudio), afin que les dossiers soient créés au bon endroit.
:::
...
Le package [`rio`](https://gesistsa.github.io/rio/) simplifie grandement l’importation et l’exportation de fichiers de données dans R avec deux fonctions principales :

-   [`import()`](https://gesistsa.github.io/rio/reference/import.html) → Lire un fichier
-   [`export()`](https://gesistsa.github.io/rio/reference/export.html) → Sauvegarder un objet R

De nombreux [formats supportés](https://gesistsa.github.io/rio/index.html#supported-file-formats) : `.csv`, `.xlsx`, `.rda`, `.sav`, etc.
...
-   La fonction [`class()`](https://rdrr.io/r/base/class.html) permet de vérifier la classe d’un objet ou d’une variable (colonne) contenue dans un tableau de données :
...
| `"tbl_df"` | `tibble(x= 1:3)` | Tableau de données moderne du package [`tibble`](https://tibble.tidyverse.org/) |
| `"list"` | `list(a = 1, b = "x")` | Liste d’objets variés |

## Une variable = un objet avec sa classe

-   Une variable (colonne) d'un [`data.frame`](https://rdrr.io/r/base/data.frame.html) (ex.: `d$age`) est elle-même un objet et a sa classe propre :
...
-   Certaines fonctions s’adaptent à la classe : [`summary()`](https://rdrr.io/r/base/summary.html), [`plot()`](https://rdrr.io/r/base/plot.html), [`print()`](https://rdrr.io/r/base/print.html) ne donnent pas les mêmes résultats selon l’objet :
...
| Fonction | Effet |
|-----------------------|-------------------------------------------------|
| [`as.numeric()`](https://rdrr.io/r/base/numeric.html) | Convertit en nombre |
| [`as.character()`](https://rdrr.io/r/base/character.html) | Convertit en texte |
| [`as.logical()`](https://rdrr.io/r/base/logical.html) | Convertit en logique (`TRUE` / `FALSE`) |
| [`as.factor()`](https://rdrr.io/r/base/factor.html) | Convertit en facteur |
| [`as.Date()`](https://rdrr.io/r/base/Date.html) | Convertit en date |
| [`as_tibble()`](https://tibble.tidyverse.org/reference/as_tibble.html) | Convertit en [`tibble`](https://tibble.tidyverse.org/) |
...
Le [`tibble`](https://tibble.tidyverse.org/index.html) possède plusieurs classes ("tbl_df", "tbl", "data.frame"), ce qui lui permet d’hériter à la fois de l’affichage amélioré des *tibbles* et de la compatibilité totale avec les fonctions traditionnelles pour les *data frames*.
...
La fonction [`typeof()`](https://rdrr.io/r/base/typeof.html) indique **le type interne** de l’objet (comment il est réellement stocké en mémoire).
...
-   C’est un objet **bidimensionnel** :
    -   [`ncol()`](https://rdrr.io/r/base/nrow.html) renvoie le nombre de colonnes\
    -   [`nrow()`](https://rdrr.io/r/base/nrow.html) renvoie le nombre de lignes\
    -   [`dim()`](https://rdrr.io/r/base/dim.html) renvoie les dimensions sous forme (lignes, colonnes)

## Explorer la structure d’un *data frame*

-   [`names()`](https://rdrr.io/r/base/names.html) affiche les noms des variables (colonnes).
-   [`str()`](https://rdrr.io/r/utils/str.html) renvoie un aperçu détaillé de la structure du *data frame* :
    -   liste des variables\
...
-   Appuyez sur <kbd>Tab ↹</kbd> pour afficher et sélectionner les variables disponibles.
...
Pour les objets bidimensionnels (2D) comme les *data frames*,\
les crochets `[ , ]` permettent d’accéder aux données, par une indexation bidimensionnelle (lignes et colonnes) :

-   Syntaxe : `df[i, j]`
    -   `df` : data frame
    -   `i` : lignes\
    -   `j` : colonnes
-   Les coordonnées correspondent aux **positions** (1^ère^ ligne, 2^ème^ ligne, etc., et 1^ère^ colonne, 2^ème^ colonne, etc.).\
...
-   Cliquez sur l’icône du balai 🧹 dans le panneau **Environment** pour tout effacer.\
...
::: {.callout-tip title="Astuce : Relancer complètement R"}
...
Raccourci clavier : <kbd style="font-size: 0.8em;">Ctrl</kbd> + <kbd style="font-size: 0.8em;">Shift</kbd> + <kbd style="font-size: 0.8em;">F10</kbd> (Windows / Linux) ou <kbd style="font-size: 0.8em;">Cmd</kbd> + <kbd style="font-size: 0.8em;">Shift</kbd> + <kbd style="font-size: 0.8em;">F10</kbd> (macOS)
...
-   [Introduction à R et au tidyverse](https://juba.github.io/tidyverse/) : Très bonne introduction en français

-   [R for Data Science (2e)](https://r4ds.hadley.nz/) : Introduction complète et progressive à la science des données avec R

-   [Awesome R Learning Resources](https://github.com/iamericfletcher/awesome-r-learning-resources) : Liste des packages/travaux/etc. géniaux dans R

-   [Advanced R](https://adv-r.hadley.nz/index.html) : Exploration approfondie du langage R

-   [The Art of R Programming](https://nostarch.com/artofr.htm) : Ouvrage de référence sur la programmation avancée et les bonnes pratiques en R (non disponible en libre accès)
